...
export class Game {
    ...
    // 🔥 ВАЖНО — сохраняем последний размер
    private lastPlayerMass = 0;
    ...
    private update(dt: number): void {
        ...
        const totalMass = this.world.playerCells.reduce((sum, c) => sum + c.radius, 0);
        this.lastPlayerMass = Math.max(this.lastPlayerMass, totalMass);

        if (this.world.playerCells.length > 0) {
            this.world.player = this.world.playerCells.reduce((best, cell) =>
                cell.radius > best.radius ? cell : best
            );
        }
  ...
  `Твой размер: ${Math.round(this.lastPlayerMass)}\nСамый крутой язык: ${top?.name ?? 'Неизвестно'}`;
            }
    ...
    private render(): void {
        this.camera.update(this.world.getPlayerFocus(), this.fixedDt);
    ...
    private updateHud(): void {
        ...
        if (this.massValue) {
            this.massValue.textContent = `${Math.round(totalPlayerMass)}`;
        }
        ...
        if (this.leaderboardList) {
            ...
            this.leaderboardList.innerHTML = top
                .map((entity) => `<li>${entity.name} (${Math.round(entity.radius)})</li>`)
                .join('');
        }
    }
    ...
    private drawMinimap(): void {
        ...
        for (const entity of this.world.entities) {
            ...
            ctx.arc(
                ...
                Math.max(2, entity.radius * sx * 0.6),
                0,
                Math.PI * 2
            );
    ...
    private splitPlayer(direction: Vector2): void {
        ...
        const splitSource = this.world.playerCells.reduce((best, cell) =>
            cell.radius > best.radius ? cell : best
        );
        ...
        const newCell = new Player({
            ...
            name: '',
            position: {
                x: splitSource.position.x + dir.x * newRadius * 1.9,
                y: splitSource.position.y + dir.y * newRadius * 1.9
            },
            radius: newRadius,
        ...
        newCell.velocity.x = dir.x * 320;
        newCell.velocity.y = dir.y * 320;
